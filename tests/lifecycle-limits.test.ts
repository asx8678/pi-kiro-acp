...
async function bounded<T>(promise: Promise<T>, ms = 2500): Promise<T> {
    ...
    try { return await Promise.race([promise, new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error('Test deadline exceeded')), ms); })]); }
    finally { clearTimeout(timer); }
}

for (const timing of ['immediate', 'late'] as const) test(`payload hook abort observes its ${timing} rejection`, timeout, async () => {
...
test('shutdown does not await a stalled observer and late rejection cannot dispatch events', timeout, async () => {
    const c = config(), r = new ProviderRuntime(c), entered = deferred<void>(), gate = deferred<void>();
    let callbacks = 0;
    try {
    ...
    } finally { gate.resolve(); await cleanup(r, c); }
});

test('closing skips stalled observers but drains already-received final accounting', timeout, async () => {
    const c = config(), entered = deferred<void>(), gate = deferred<void>();
    const s = new V3Session(c, c.stateDir, new Catalog([]), async () => { entered.resolve(); await gate.promise; });
    const credits: number[] = [], tokens: number[] = [];
    s.onEvent = async event => { if (event.kind === 'credits') credits.push(event.report.total!); if (event.kind === 'tokens') tokens.push(event.report.counts.inputTokens!); };
    ...
    try {
        ...
        assert.deepEqual(credits, [0.25]); assert.deepEqual(tokens, [10]);
    } finally { gate.resolve(); await s.close(); await prompt; fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});
...
test('observer rejection preserves credit and token reports from the same frame', timeout, async () => {
    ...
    try {
        ...
        await bounded(s.close()); await bounded(prompt);
        assert.deepEqual(credits, [0.25]); assert.deepEqual(tokens, [10]);
    } finally { await s.close(); await prompt; fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});

test('discovery waits for the delayed initial catalog without sending inference', timeout, async () => {
    const c = config(); c.cli.prefixArgs.push('--delayed-catalog', '--tags-inventory');
...
test('delayed model and effort confirmations are awaited without implicit fallback', timeout, async () => {
    const c = config(); c.cli.prefixArgs.push('--delayed-selection');
    ...
    try {
        ...
        assert.equal(s.selected().model, 'test-opus'); assert.equal(s.selected().effort, 'high');
        await assert.rejects(s.select('not-advertised'), /did not advertise/);
    } finally { await s.close(); fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});

test('missing catalog readiness has a bounded timeout', timeout, async () => {
    const c = config(); c.cli.prefixArgs.push('--missing-catalog'); c.cli.rpcTimeoutMs = 1500;
    const s = new V3Session(c, c.stateDir, new Catalog([]));
    try { await assert.rejects(s.start('Offline timeout test', undefined), error => error instanceof BridgeError && error.code === 'TIMEOUT' && /catalog/.test(error.message)); }
    finally { await s.close(); fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});

for (const action of ['abort', 'close'] as const) test(`catalog readiness is cancellable by ${action}`, timeout, async () => {
    const c = config(); c.cli.prefixArgs.push('--missing-catalog');
    const s = new V3Session(c, c.stateDir, new Catalog([])), ctl = new AbortController(), activated = deferred<void>();
    const request = s.rpc.request.bind(s.rpc);
    s.rpc.request = async (method, params, options) => { const result = await request(method, params, options); if (method === 'session/set_mode') activated.resolve(); return result; };
    ...
    try { await bounded(activated.promise); if (action === 'abort') ctl.abort(); else await bounded(s.close()); await bounded(rejected); }
    finally { ctl.abort(); await s.close(); await rejected; fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});

for (const bound of ['count', 'bytes'] as const) test(`ACP notification backlog enforces its ${bound} bound with a slow observer`, timeout, async () => {
    const c = config(); c.limits.maxQueuedEvents = bound === 'count' ? 2 : 8;
    c.limits.maxPromptBytes = 1024; c.limits.maxFrameBytes = 131072;
    ...
    try {
        await s.start('Offline backlog test', undefined); send(); await bounded(entered.promise);
        send(); send();
...
for (const bound of ['count', 'bytes'] as const) for (const corrected of [false, true]) test(`ACP ${bound} overflow preserves ${corrected ? 'corrected' : 'received'} accounting and enforces the budget`, timeout, async () => {
    const c = config(); c.limits.maxQueuedEvents = bound === 'count' ? 2 : 8;
    c.limits.maxPromptBytes = 1024; c.limits.maxFrameBytes = 131072; c.budget.dailyCredits = 0.25;
    ...
    s.onEvent = async event => {
        if (event.kind === 'prompt_start') { promptId = event.promptId; ledger.start(promptId, 'auto', event.startedAt); }
        if (event.kind === 'prompt_end') ledger.finish(event.promptId);
        if (event.kind === 'credits') { ledger.record(event.report); credits.push(event.report.total!); }
        if (event.kind === 'tokens') { ledger.recordTokens(event.report); tokens.push(event.report.counts.inputTokens!); }
    };
    const report = (total: number, inputTokens?: number, padded = false) => s.rpc.onNotification('session/update', {
    ...
    try {
        ...
        if (corrected) for (let i = 0; i < 32; i++) report(0.75, 7);
        await bounded(s.close());
    ...
    } finally { gate.resolve(); await s.close(); await prompt; ledger.close(); journal.close(); fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});

test('adapter enforces the configured queue limit around a host-provided stream', timeout, async () => {
    const c = config(); c.limits.maxQueuedEvents = 2;
    const r = new ProviderRuntime(c, undefined, () => new LocalStream(100000));
    try {
        const stream = r.generate(model(c), context('plain output'));
        const result = await stream.result();
        assert.equal(result.stopReason, 'error'); assert.match(result.errorMessage!, /queue/);
...
test('completion queue overflow ends the stream and releases admission for the next generation', timeout, async () => {
    const c = config(); c.limits.maxQueuedEvents = 3; c.admission.maxActive = 1;
    const r = new ProviderRuntime(c, undefined, () => new LocalStream(100000));
    try {
        ...
        assert.equal(result.stopReason, 'error'); assert.match(result.errorMessage!, /LIMIT:.*queue/);
        const events = []; for await (const event of stream) events.push(event);
...
test('bounded host streams release capacity as events are consumed', timeout, async () => {
    ...
    try {
        const consumed = (async () => { const events = []; for await (const event of stream) events.push(event); return events; })();
        for (let i = 0; i < 20; i++) { writer.chunk('text', 'x'); await tick(); }
        writer.finish(); const events = await consumed;
        assert.equal(events.filter(e => e.type === 'text_delta').length, 20);
        assert.equal((await stream.result()).stopReason, 'stop');
        assert.equal(events.filter(e => e.type === 'done').length, 1);
    } finally { fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});
