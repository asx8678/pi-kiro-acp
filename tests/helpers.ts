...
import type { Assistant, EffectiveContext, PiStream, ToolCall } from '../src/types.js';
...
export function model(c: Config, id = 'test-opus') { return toModel({ id, name: id }, c); }
export function context(prompt = 'CALL_TOOL'): EffectiveContext {
    return { messages: [
...
export async function collect(stream: PiStream): Promise<Assistant> { for await (const _ of stream) { } return stream.result(); }
export function addResult(ctx: EffectiveContext, answer: Assistant, isError = false): void {
    ctx.messages.push(answer);
    for (const call of answer.content.filter((b): b is ToolCall => b.type === 'toolCall'))
...
export async function cleanup(runtime: ProviderRuntime, c: Config): Promise<void> { await runtime.close(); fs.rmSync(c.stateDir, { recursive: true, force: true }); }
