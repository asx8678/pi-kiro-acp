...
test('cancellation escalates the process group after its leader exits', { ...timeout, skip: process.platform === 'win32' }, async () => {
    ...
    rpc.onNotification = (method, params) => {
        if (method === 'fixture/child_ready' && object(params) && typeof params.pid === 'number') descendant = params.pid;
    };
    try {
        rpc.start();
...
for (const timing of ['idle', 'pre-send'] as const) test(`model drift at ${timing} is rejected before sending a prompt`, timeout, async () => {
    const c = config(), s = new V3Session(c, c.stateDir, new Catalog([]));
    let promptRequests = 0;
    const request = s.rpc.request.bind(s.rpc);
    s.rpc.request = (method, params, options) => { if (method === 'session/prompt') promptRequests++; return request(method, params, options); };
    const drift = async () => {
    ...
    try {
        ...
        else s.onEvent = async event => { if (event.kind === 'prompt_start') await drift(); };
...
test('select cannot change the model while a prompt owns the session', timeout, async () => {
    const c = config(), s = new V3Session(c, c.stateDir, new Catalog([]));
    try {
        await s.start('Offline selection ownership check', undefined); await s.select('test-opus');
        s.onEvent = async event => { if (event.kind === 'prompt_start') await assert.rejects(s.select('auto'), { code: 'BUSY' }); };
...
test('reused runtime refuses an unsolicited idle model change instead of mislabelling Auto', timeout, async () => {
    const c = config(); c.cli.prefixArgs.push('--idle-model-change');
    ...
    try {
        ...
        await until(() => (r.status().sessions as { selected: { model: string } }[]).some(s => s.selected.model === 'auto'));
...
for (const retain of [false, true]) test(`runtime accounting ${retain ? 'retains opted-in' : 'does not persist default'} task text`, timeout, async () => {
    const c = config(); c.reporting.retainTaskExcerpts = retain;
    ...
    try {
        ...
        r.credits.noteResult(sentinels[1]!, 'completed'); r.credits.finish('privacy-prompt'); r.credits.endTask(id);
        ...
        for (const sentinel of sentinels) assert.equal(database.includes(Buffer.from(sentinel)), retain);
    } finally { await cleanup(r, c); }
});
...
test('a default writer does not copy legacy excerpts into a late accounting report', () => {
    ...
    try {
        ...
        writer.start('late-report', 'auto', Date.now(), id); writer.noteResult(marker); writer.endTask(id);
        ...
        assert.ok(appended.length > 0); assert.ok(!appended.includes(marker));
    ...
    } finally { safe.close(); writer.close(); journal.close(); fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});
