...
import type { Assistant } from '../src/types.js';
import { config, model, context, collect, addResult, cleanup } from './helpers.js';

const timeout = { timeout: 15000 };
async function until(ready: () => boolean): Promise<void> {
    const deadline = Date.now() + 6000;
    while (!ready()) { assert.ok(Date.now() < deadline, 'Fixture state deadline exceeded'); await delay(10); }
}
function owner(c: ReturnType<typeof config>) {
    ...
    const ready = deferred<void>(), answer = deferred<Assistant>(), closed = deferred<void>(), claimed = deferred<void>();
    child.on('message', message => {
        if (!object(message)) return;
        if (message.type === 'ready') ready.resolve();
        if (message.type === 'claimed') claimed.resolve();
        if (message.type === 'answer') answer.resolve(message.answer as Assistant);
        if (message.type === 'failure') { ready.reject(new Error(String(message.message))); answer.reject(new Error(String(message.message))); }
    });
    ...
    void ready.promise.catch(() => {}); void answer.promise.catch(() => {});
...
test('binding reservation is atomic, fenced, and distinct from account admission', () => {
    ...
    try {
        ...
        const unrelated = second.reserveBinding('other-conversation'); unrelated();
        release();
        ...
        next();
        first.reserveBinding('conversation'); first.close();
        const afterReload = second.reserveBinding('conversation'); afterReload();
...
test('a reservation survives a live worker but is reclaimable after an actual OS-process crash', timeout, async () => {
    ...
    try {
        await worker.ready; worker.child.send('claim'); await worker.claimed;
        assert.throws(() => j.reserveBinding('orphaned'), { code: 'BUSY' });
        worker.child.kill('SIGKILL'); await worker.closed;
        const release = j.reserveBinding('orphaned'); release();
    } finally {
        if (worker.child.connected) worker.child.send('close');
        await worker.closed; j.close(); fs.rmSync(c.stateDir, { recursive: true, force: true });
    }
});
...
test('shared conversation excludes a competing OS process before startup and tool dispatch', timeout, async () => {
    ...
    c.cli.prefixArgs.push('--catalog-gate', gate); c.cli.rpcTimeoutMs = 10000;
    const a = owner(c); let b: ReturnType<typeof owner> | undefined;
    try {
        await a.ready; a.child.send('run');
        await until(() => fs.existsSync(gate + '.ready'));
        b = owner(c); await b.ready; b.child.send('run');
        ...
        await Promise.all([a, b].filter(x => x !== undefined).map(async x => {
            if (x.child.connected) x.child.send('close');
            await x.closed;
        }));
...
for (const action of ['abort', 'idle-sweep', 'invalidate'] as const) test(`startup is active during ${action}`, timeout, async () => {
    const c = config(), gate = c.stateDir + '/catalog-gate';
    c.cli.prefixArgs.push('--catalog-gate', gate); c.cli.rpcTimeoutMs = 10000;
    if (action === 'idle-sweep') c.sessions.idleTtlMs = 25;
    ...
    try {
        ...
        if (action === 'abort') {
            ...
            if (action === 'invalidate') await r.invalidate();
            else await delay(100);
            assert.equal((r.status().sessions as { phase: string }[])[0]?.phase, 'STARTING');
            fs.writeFileSync(gate, 'release');
...
test('shutdown closes the journal and allows safe recovery even when usage logging fails', timeout, async () => {
    ...
    try {
        ...
        if (fs.existsSync(log) && fs.statSync(log).isDirectory()) fs.rmdirSync(log);
        if (fs.existsSync(log + '.saved')) fs.renameSync(log + '.saved', log);
        await next?.close(); await r.close().catch(() => {});
        if (!r.journal.closed) { r.credits.close(); r.journal.close(); }
        fs.rmSync(c.stateDir, { recursive: true, force: true });
    }
});

for (const timing of ['idle', 'pre-send', 'in-flight'] as const) test(`requested effort cannot drift at ${timing}`, timeout, async () => {
    const c = config(), s = new V3Session(c, c.stateDir, new Catalog([]));
    let prompts = 0;
    const request = s.rpc.request.bind(s.rpc);
    s.rpc.request = (method, params, options) => { if (method === 'session/prompt') prompts++; return request(method, params, options); };
    const drift = async () => {
    ...
    try {
        await s.start('Offline effort pin test', undefined); await s.select('test-opus', 'high');
        if (timing === 'idle') await drift();
        if (timing === 'pre-send') s.onEvent = async event => { if (event.kind === 'prompt_start') await drift(); };
...
for (const variant of ['renamed', 'removed'] as const) test(`effort pin follows the advertised option ID when ${variant}`, timeout, async () => {
    const c = config(); c.cli.prefixArgs.push('--renamed-effort');
    const s = new V3Session(c, c.stateDir, new Catalog([]));
    try {
        await s.start('Offline effort schema test', undefined); await s.select('test-opus', 'high');
        ...
        if (variant === 'removed') changed.configOptions = changed.configOptions.filter(o => object(o) && o.category !== 'thought_level');
        s.rpc.onNotification('session/update', { sessionId: s.sessionId, update: { sessionUpdate: 'config_option_update', ...changed } });
...
test('unspecified effort stays automatic rather than retaining an old explicit pin', timeout, async () => {
    ...
    try {
        ...
        await s.select('test-opus', 'high'); await s.select('test-opus');
...
test('fragmented NDJSON only measures/scans new input, not the accumulated frame', () => {
    ...
    try {
        ...
        for (let offset = 0; offset < frame.length; offset += 256) parser.push(frame.subarray(offset, offset + 256));
        parser.end();
        assert.equal(frames, 1);
        assert.ok(countedBytes + scannedCharacters <= frame.length * 3,
            `Rescanned ${countedBytes} bytes and ${scannedCharacters} characters for a ${frame.length}-byte frame`);
    } finally { Buffer.byteLength = byteLength; String.prototype.indexOf = indexOf; }
});
...
test('NDJSON permits only the initial BOM and bounds retained buffer capacity', () => {
    ...
    for (let i = 0; i < frame.length; i += 127) parser.push(frame.subarray(i, i + 127));
    assert.deepEqual(frames, [message]);
    assert.ok((parser as unknown as { buffer: Buffer }).buffer.length <= 65536);
...
test('NDJSON frame limits and CRLF remain correct across every byte boundary', () => {
    ...
    const frames: unknown[] = [], lengths: number[] = [];
    const parser = new Ndjson(size, (frame, bytes) => { frames.push(frame); lengths.push(bytes); });
    const input = Buffer.from(line + '\r\n' + line + '\n');
    for (const byte of input) parser.push(Uint8Array.of(byte));
    parser.end();
    assert.equal(frames.length, 2); assert.deepEqual(lengths, [size, size]);
    assert.throws(() => new Ndjson(size - 1, () => {}).push(Buffer.from(line + '\n')), /limit/);
    assert.throws(() => new Ndjson(10, () => {}).push(Buffer.from(' '.repeat(11) + '\n')), /limit/);
    const invalid = new Ndjson(100, () => {}); invalid.push(Uint8Array.of(0xe2));
