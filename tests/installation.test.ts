...
test('simultaneous first installs converge on one complete persisted ID', async t => {
    ...
    const results = await Promise.all(Array.from({ length: 8 }, () => exec(process.execPath,
        ['--input-type=module', '-e', code, config.stateDir], { timeout: 10000 })));
...
test('npm archives exclude local installation state even inside included source directories', { skip: process.platform === 'win32' }, t => {
    ...
    assert.equal(result.status, 0, result.stderr);
    const files = (JSON.parse(result.stdout)[0].files as { path: string }[]).map(file => file.path);
