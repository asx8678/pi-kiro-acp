...
const credit = (total: number) => ({ promptId: 'prompt', total, delta: total, reports: 1, source: 'turn_completion' as const });
...
test('cached accounting observes own and other-connection corrections immediately', () => {
    ...
    try {
        f.ledger.record(credit(0.5)); assert.equal(f.ledger.snapshot().reportedCredits, 0.5);
        worker.record(credit(0.25)); assert.equal(f.ledger.snapshot().reportedCredits, 0.25);
        f.ledger.record(credit(0)); assert.equal(f.ledger.snapshot().reportedCredits, 0);
    ...
    } finally { worker.close(); other.close(); f.close(); }
});
...
test('cached view models cannot be poisoned by mutation or rolled-back writes', () => {
    ...
    try {
        ...
        const view = f.ledger.snapshot(); view.reportedCredits = 0;
...
test('aggregate caching stays bounded and warm reads only check the data revision', () => {
    const f = fixture(), prepare = f.journal.db.prepare.bind(f.journal.db), statements: string[] = [];
    f.journal.db.prepare = sql => { statements.push(sql); return prepare(sql); };
    try {
        f.ledger.snapshot(); f.ledger.sessionTokens('session'); f.ledger.dashboard();
        statements.length = 0;
        f.ledger.snapshot(); f.ledger.sessionTokens('session'); f.ledger.dashboard();
        assert.equal(statements.length, 3);
        assert.ok(statements.every(sql => sql.includes('pragma_data_version')));
        for (let i = 0; i < 200; i++) f.ledger.sessionTokens('absent-' + i);
        const cache = (f.ledger as unknown as { cache: Map<string, unknown> }).cache;
...
test('admission budget checks never aggregate all-time history', () => {
    ...
    f.journal.db.prepare = sql => { statements.push(sql); return prepare(sql); };
    try {
        f.c.budget.dailyCredits = 0.4; f.ledger.record(credit(0.5)); statements.length = 0;
        assert.throws(() => f.ledger.assertAvailable(), /budget reached/);
        assert.equal(statements.length, 1); assert.match(statements[0]!, /started_at>=\?/);
        f.ledger.record(credit(0)); assert.doesNotThrow(() => f.ledger.assertAvailable());
    } finally { f.close(); }
});
...
test('indexed session token totals retain orphan fallback without double counting', () => {
    ...
    let tokenQuery = '';
    f.journal.db.prepare = sql => { if (sql.includes('COUNT(u.prompt_id)')) tokenQuery = sql; return prepare(sql); };
    try {
        ...
        assert.match(plan, /credit_tasks_session/); assert.match(plan, /credit_prompts_owner/);
...
test('indexed monthly aggregation respects DST and skipped civil days', () => {
    for (const [month, zone, skipped] of [['2026-03', 'Europe/Paris', ''], ['2026-10', 'Europe/Paris', ''], ['2011-12', 'Pacific/Apia', '2011-12-30']] as const) {
        const f = fixture(zone);
        try {
            const bounds = monthDayRanges(month, zone);
            if (skipped) { const day = bounds.find(day => day.day === skipped)!; assert.equal(day.start, day.end); }
            ...
            if (dst) assert.equal((dst.end - dst.start) / 3600000, month === '2026-03' ? 23 : 25);
        } finally { f.close(); }
    }
});
...
test('dashboard render and resize perform no SQL and reload refreshes selected tokens', async () => {
    const f = fixture(), prepare = f.journal.db.prepare.bind(f.journal.db);
    let queries = 0, panel: UsagePanel | undefined;
    const ui: UsageUiPort = {
        Input: class { render() { return []; } invalidate() {} handleInput() {} },
    ...
    const host: HostContext = { cwd: f.c.stateDir, mode: 'tui', ui: {
        notify() {}, custom: async factory => { panel = factory({ terminal: { rows: 40 }, requestRender() {} }, { fg: (_color, text) => text, bold: text => text }, undefined, () => {}); return undefined as never; },
    } };
    ...
    try {
        ...
        await tick(); panel!.handleInput!('return');
        let before = queries;
        assert.match(panel!.render(100).join('\n'), /123/); panel!.invalidate(); panel!.render(80); panel!.render(120);
        ...
        panel!.handleInput!('r'); await tick(); before = queries;
        assert.match(panel!.render(100).join('\n'), /999/);
        assert.equal(queries, before);
    } finally { panel?.dispose?.(); f.close(); }
});
