...
test('widget updates coalesce and pending refreshes are cancelled on shutdown', async () => {
    const c = config(), file = c.stateDir + '/config.json', old = process.env.PI_KIRO_ACP_CONFIG;
    writePrivateJson(file, c); process.env.PI_KIRO_ACP_CONFIG = file;
    const handlers = new Map<string, (event: Obj, ctx: HostContext) => unknown>();
    const pi: PiPort = { registerProvider() {}, registerCommand() {}, on: (event, handler) => { handlers.set(event, handler); } };
    const r = await installExtension(pi, { createAssistantMessageEventStream: () => new LocalStream(), getCurrentSystemPrompt: fallbackExtractors.system, getCurrentTools: fallbackExtractors.tools });
    let reads = 0, updates = 0;
    const snapshot = r.credits.snapshot.bind(r.credits);
    r.credits.snapshot = now => { reads++; return snapshot(now); };
    const host: HostContext = { cwd: c.stateDir, model: { provider: 'kiro-acp', id: 'auto' }, ui: { notify() {}, setStatus: (_key, text) => { if (text) updates++; } } };
    try {
        for (let i = 0; i < 20; i++) { handlers.get('before_provider_request')!({}, host); r.metrics.onCredits?.(); }
        ...
        await new Promise(resolve => setTimeout(resolve, 100));
        assert.equal(reads, 1); assert.equal(updates, 1);
    } finally { await handlers.get('session_shutdown')!({}, host); await cleanup(r, c); if (old === undefined) delete process.env.PI_KIRO_ACP_CONFIG; else process.env.PI_KIRO_ACP_CONFIG = old; }
});
