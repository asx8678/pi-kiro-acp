...
for (const owner of ['active', 'restarted'] as const) test(`budget cutoff records the ${owner} owner's completed tool before refusing continuation`, timeout, async () => {
    const c = config(); c.budget.dailyCredits = 1;
    ...
    try {
        ...
        addResult(ctx, first);
        if (owner === 'restarted') { await r.close(); r = new ProviderRuntime(c); }
...
test('a clean reset followed by restart preserves unresolved recovery identity', timeout, async () => {
    ...
    } finally { await first.close(); await second?.close(); fs.rmSync(c.stateDir, { recursive: true, force: true }); }
});
...
test('a live legacy owner blocks the provider before a new prompt is sent', timeout, async () => {
    ...
    try {
        ...
        assert.equal(r.metrics.toolCalls, 0);
    } finally { old.close(); await cleanup(r, c); }
});
...
test('cancellation kills an initialized CLI with blocked stdin and ignored SIGTERM', timeout, async () => {
    const c = config(); c.cli.prefixArgs.push('--stall-after-select'); c.cli.cancelGraceMs = 150;
    ...
    try {
        ...
        const pid = session.rpc.pid;
        if (pid && alive(pid)) { try { process.kill(process.platform === 'win32' ? pid : -pid, 'SIGKILL'); } catch { /* gone */ } }
        await session.close(); await prompt;
