...
import type { EffectiveContext, ToolCall } from '../src/types.js';
import { config, context, model, collect, addResult, cleanup } from './helpers.js';

const timeout = { timeout: 15000 };
const outputContext = (prompt = 'CALL_TOOL', name = 'decision_output'): EffectiveContext => ({
...
test('structured completion shares task attribution but never takes over a held main conversation', timeout, async () => {
    const c = config(); c.admission.maxActive = 1;
    ...
    try {
        ...
        assert.equal((result.content.find(b => b.type === 'toolCall') as ToolCall).name, 'decision_output');
        ...
        assert.equal((r.status().sessions as unknown[]).length, 1, 'the parent session survives');
        addResult(ctx, first);
...
test('repeated result-only completions leave no held effects, resident sessions or admission slots', timeout, async () => {
    const c = config(); c.sessions.maxResident = 1;
    ...
    try {
        for (let i = 0; i < 3; i++) {
            ...
            assert.equal(result.stopReason, 'toolUse');
            assert.deepEqual((result.content.find(b => b.type === 'toolCall') as ToolCall).arguments, { value: 'hello', iteration: 0 });
            assert.equal((r.status().sessions as unknown[]).length, 0);
...
test('structured completion rejects ambiguous schemas and invalid deadlines before inference', timeout, async () => {
    const c = config(), r = new ProviderRuntime(c);
    try {
        for (const tools of [[], [...outputContext().tools!, { ...outputContext().tools![0] as object, name: 'another' }]])
...
for (const action of ['abort', 'shutdown', 'timeout'] as const) test(`structured completion cleans up on ${action} without an effect journal record`, timeout, async () => {
    ...
    try {
        ...
        if (action !== 'timeout') {
            ...
            while (r.metrics.generations === 0) { assert.ok(Date.now() < deadline); await delay(10); }
...
test('structured output bytes are bounded without publishing a partial decision', timeout, async () => {
    const c = config(); c.limits.maxOutputBytes = 30;
    const r = new ProviderRuntime(c);
    try {
