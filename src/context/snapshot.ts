...
export const fallbackExtractors: Extractors = {
    system(messages) {
...
export function projectMessage(raw: unknown): Obj {
    if (!object(raw))
    ...
    const out: Obj = { role, content: blocks(raw.content) };
    if (role === 'toolResult') {
    ...
    return JSON.parse(canonical(out)) as Obj;
}
export function snapshot(context: EffectiveContext, extractors: Extractors = fallbackExtractors, noTools = false, maxBytes = 200000): Snapshot {
    ...
    const tools = rawTools.map(raw => {
        ...
        return JSON.parse(canonical({ name: raw.name, description: raw.description, parameters: raw.parameters })) as Tool;
    });
...
export function appendInput(messages: Obj[]): string {
    if (messages.length === 1 && messages[0]?.role === 'user')
        return text(messages[0].content);
