...
export class Metrics {
    ...
    private creditTotals = new Map<string | undefined, number>();
    ...
    creditStatus(usage = this.ledger?.snapshot()): string {
        if (this.ledger && usage) {
            ...
            if (usage.exhausted) parts.push('daily limit reached');
            else if (usage.warning) parts.push('daily warning');
            ...
            if (this.context) parts.push(`Context ${this.context.percent.toFixed(1)}% (${this.context.model})`);
            return parts.join(' · ');
        }
