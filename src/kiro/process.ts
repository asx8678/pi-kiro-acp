...
export function signalOwnedProcess(child: ChildProcess, signal: NodeJS.Signals): void {
    if (!child.pid) return;
    try {
...
function groupExists(child: ChildProcess): boolean {
    ...
    catch (error) { return !object(error) || error.code !== 'ESRCH'; }
}
/** A leader closing its pipes does not prove its descendants exited. */
export async function closeOwnedProcess(child: ChildProcess, closed: Promise<unknown>, graceMs: number): Promise<void> {
    signalOwnedProcess(child, 'SIGTERM');
    const escalated = deferred<void>();
    const timer = setTimeout(() => { signalOwnedProcess(child, 'SIGKILL'); escalated.resolve(); }, Math.max(0, graceMs));
    try {
        await closed;
        if (groupExists(child)) await escalated.promise;
