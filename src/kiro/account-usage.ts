...
const label = (value: unknown, fallback: string): string => typeof value === 'string' ? value.replace(/[\x00-\x1f\x7f-\x9f]/g, ' ').slice(0, 160) : fallback;
function allowance(row: Obj, fallback: string, totalField = 'total'): CreditAllowance | undefined {
...
export function parseAccountUsage(raw: unknown, now = Date.now()): AccountUsage {
    ...
    const packs = (rows: unknown, name: string) => Array.isArray(rows) ? rows.filter(object).filter(row => row.isActive !== false).flatMap(row => { const item = allowance(row, name); return item ? [item] : []; }).slice(0, 100) : [];
    return {
