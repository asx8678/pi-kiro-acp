...
export const creditNumber = (value: number): string => value > 0 && value < 0.0001 ? '<0.0001' : new Intl.NumberFormat('en-US', { maximumFractionDigits: 4 }).format(value);
...
function dateLabel(day: string, weekday = false): string {
    return new Intl.DateTimeFormat('en-GB', { timeZone: 'UTC', ...(weekday ? { weekday: 'short' as const } : {}), day: 'numeric', month: 'short' }).format(new Date(`${day}T12:00:00Z`));
}
...
interface ActivityRow { id: string; label: string; time?: string; totals: UsageTotals; detail: string; note?: string }
...
class Dashboard implements UsagePanel {
    ...
    constructor(private ledger: CreditLedger, selected: string | undefined, private refresh: (force?: boolean) => Promise<AccountUsage>, private ui: UsageUiPort,
        private tui: UsageTerminal, private theme: UsageTheme, private done: (value: void) => void) {
        this.data = ledger.dashboard(selected);
        this.selection = this.data.selected;
        if (!validDay(this.selection)) this.cursor = this.days().findIndex(day => day.day === this.data.today);
    ...
    private reload(): void {
        ...
        try { this.data = this.ledger.dashboard(this.selection); }
    ...
    private async refreshPlan(force: boolean): Promise<void> {
        ...
        finally { this.refreshing = false; this.reload(); }
    }
    private navigate(selected: string): void {
        try {
            const data = this.ledger.dashboard(selected);
            this.selection = data.selected; this.data = data; this.cursor = data.selected === data.currentMonth ? Math.max(0, this.days().findIndex(day => day.day === data.today)) : 0; this.offset = 0;
            this.session = undefined; this.sessionName = undefined; this.error = ''; this.input = undefined;
        } catch (error) { this.error = publicError(error); }
        this.tui.requestRender();
    }
    ...
    handleInput(data: string): void {
        if (this.input) { this.input.handleInput(data); this.tui.requestRender(); return; }
        const key = (name: string) => this.ui.matchesKey(data, name);
        if (key('escape') || data === 'b') {
            if (this.session) {
                const id = this.session; this.session = undefined; this.sessionName = undefined;
                this.cursor = Math.max(0, this.data.sessions.findIndex(session => session.id === id)); this.offset = 0;
            }
            else if (validDay(this.selection)) {
                const day = this.selection; this.navigate(day.slice(0, 7));
                this.cursor = Math.max(0, this.days().findIndex(row => row.day === day));
            }
            else this.done();
        } else if (data === 'q' || key('ctrl+c')) this.done();
            ...
            this.cursor = Math.max(0, this.days().findIndex(day => day.day === selected)); this.offset = 0;
        }
        else if (key('left')) { try { this.navigate(shiftMonth(this.data.month, -1)); } catch (error) { this.error = publicError(error); } }
        else if (key('right')) { try { this.navigate(shiftMonth(this.data.month, 1)); } catch (error) { this.error = publicError(error); } }
        else if (data === 'd') {
        ...
        else if (key('down') || data === 'j') this.cursor = Math.min(this.rows().length - 1, this.cursor + 1);
            ...
            if (validDay(this.selection)) {
                ...
                if (row) { this.session = row.id; this.sessionName = sessionTitle(row); this.cursor = 0; this.offset = 0; }
            } else {
                const row = this.days()[this.cursor];
                if (row) this.navigate(row.day);
            }
        }
        this.tui.requestRender();
    }
    render(width: number): string[] {
        ...
        if (!dayView) {
            ...
            if (account?.lastError) header.push(row(plan ? 'Account refresh failed · showing the last balance' : 'Account refresh failed · r to retry', 'warning'));
            if (compact) {
        ...
        } else footer.push(row(inner < 50 ? '↑↓ Enter ←→ d date Esc back' : `↑↓ select   ${this.session ? '' : `Enter ${dayView ? 'tasks' : 'sessions'}   `}←→ month   d date`, 'muted'));
        ...
        if (!body.length) body.push(row('No recorded activity for this selection.', 'muted'));
...
export async function openUsageDashboard(args: string, ctx: HostContext, ledger: CreditLedger, refresh: (force?: boolean) => Promise<AccountUsage>, ui?: UsageUiPort): Promise<void> {
    ...
    await ctx.ui.custom<void>((tui, theme, _keys, done) => new Dashboard(ledger, selection, refresh, ui, tui, theme, done), { overlay: true, overlayOptions: { width: '100%' } });
}
