import type { PiPort, PiStream, Model, EffectiveContext, GenerationOptions, HostContext, Extractors } from './types.js';
...
export async function installExtension(pi: PiPort, ai: PiAiPort, tui?: UsageUiPort): Promise<ProviderRuntime> {
    ...
    const extractors: Extractors = { system: ai.getCurrentSystemPrompt, tools: ai.getCurrentTools };
    ...
    let warned = false;
    let catalogLoad: ReturnType<ProviderRuntime['discover']> | undefined;
    ...
    const stream = (model: Model, context: EffectiveContext, options?: GenerationOptions) => { checkFabric(); return runtime.generate(model, context, options); };
    ...
    const showCredits = () => {
        clearTimeout(creditUpdate); creditUpdate = undefined;
    ...
    const scheduleCredits = () => {
        if (creditUpdate || runtime.journal.closed) return;
    ...
    function bind(ctx: HostContext): void {
        runtime.setHost(ctx);
        creditsUI = ctx.ui;
        usageSessionId = ctx.sessionManager?.getSessionId?.() || runtime.journal.instance;
        scheduleCredits();
    }
    function guard(ctx: HostContext): void {
        bind(ctx);
        try { checkFabric(); } catch (e) { void ctx.abort?.(); throw e; }
    ...
    pi.on('tool_call', (event) => {
        if (event.toolName !== 'fabric_exec')
            return;
        try { checkFabric(); } catch (e) { return { block: true, reason: publicError(e) }; }
        return;
    });
    ...
    pi.on('agent_end', (event) => {
        ...
        if (object(last) && Array.isArray(last.content)) {
            const text = last.content.filter(block => object(block) && block.type === 'text' && typeof block.text === 'string').map(block => (block as { text: string }).text).join(' ');
    ...
    pi.on('agent_settled', async () => {
        await runtime.abortActive(); runtime.credits.endTask(); showCredits();
        if (!process.env.PI_FABRIC_DEPTH) void runtime.refreshAccountUsage().catch(() => {});
    });
    ...
    pi.registerCommand('usage', { description: 'Kiro credit dashboard: /usage [YYYY-MM | YYYY-MM-DD | refresh]', handler: async (args, ctx) => {
        bind(ctx);
        try { await openUsageDashboard(args, ctx, runtime.credits, force => runtime.refreshAccountUsage(force), tui, config.reporting.accountCacheMs); }
        catch (error) { ctx.ui?.notify(publicError(error), 'error'); }
    } });
    return runtime;
}
export default async function extension(pi: PiPort): Promise<void> {
    ...
    const ai = await import(moduleName) as unknown as PiAiPort;
