...
import type { Assistant, ToolCall, EffectiveContext } from './types.js';
...
`;
async function consume(stream: ReturnType<ProviderRuntime['generate']>): Promise<Assistant> {
...
async function probe(runtime: ProviderRuntime, id: string, demo = false): Promise<void> {
    ...
    const ctx: EffectiveContext = { messages: [
    ...
    for (let step = 0; step < 4; step++) {
        ...
        for (const call of result.content.filter((c): c is ToolCall => c.type === 'toolCall')) {
...
export async function main(args = process.argv.slice(2)): Promise<void> {
    ...
    const interrupt = () => { if (!interrupted) {
        interrupted = true;
        void runtime.close();
    } };
