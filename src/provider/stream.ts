...
export class LocalStream implements PiStream {
    private events: PiEvent[] = [];
    private waiters: ((value: IteratorResult<PiEvent>) => void)[] = [];
    private ended = false;
    private final = deferred<Assistant>();
    constructor(private limit = 8192) { }
    ...
    end(result?: Assistant): void { this.ended = true; if (result)
        this.final.resolve(result); this.finishWaiters(); }
    private finishWaiters(): void { for (const w of this.waiters.splice(0))
    ...
    async *[Symbol.asyncIterator](): AsyncIterator<PiEvent> {
        while (true) {
            ...
            if (this.ended)
                return;
            const next = await new Promise<IteratorResult<PiEvent>>(resolve => this.waiters.push(resolve));
            if (next.done)
                return;
            yield next.value;
        }
...
export class StreamWriter {
    ...
    private bytes = 0;
    constructor(readonly stream: PiStream, model: Model, private maxBytes: number) {
    ...
    tool(call: ToolCall): void {
        ...
        const initial: ToolCall = { ...call, arguments: {} };
    ...
    finish(reason: 'stop' | 'length' | 'toolUse' = 'stop', raw?: string): void {
        if (this.done)
            return;
        this.start();
        this.endBlock();
