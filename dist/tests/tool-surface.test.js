...
const tagSnapshot = (tags = [{ tag: '@kirocrew-core/bridge_probe', source: 'mcp' }]) => ({ sessionId: 'test-session', tags });
const catalogSnapshot = (changes = {}) => ({ sessionId: 'test-session', servers: [{
...
const tags = (a, value = tagSnapshot()) => a.observe('_kiro/tools/didChange', value);
const server = (a, value = catalogSnapshot()) => a.observe('_kiro/mcp/status', value);
...
test('tool removal, reconnect and lost provenance invalidate a confirmed surface', () => {
    for (const change of [(a) => tags(a, tagSnapshot([])), (a) => server(a, catalogSnapshot({ status: 'connecting' })), (a) => server(a, catalogSnapshot({ _meta: undefined })), (a) => server(a, { servers: [] })]) {
        const a = audit();
        server(a);
        tags(a);
        assert.equal(a.confirm(), true);
...
//# sourceMappingURL=tool-surface.test.js.map