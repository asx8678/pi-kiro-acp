...
async function until(ready) {
    ...
    while (!ready()) {
        assert.ok(Date.now() < deadline, 'Fixture readiness deadline exceeded');
        await delay(10);
    }
}
function bindingOf(r) {
    return [...r.bindings.values()][0];
}
// Only needed to clean up a pre-fix regression failure; normal tests use public close.
async function rescue(r, b) {
    if (b) {
        await bounded(b.kiro.close()).catch(() => { });
        await b.bridge.close();
        const stalled = b;
        stalled.writer?.fail(cancelled());
        stalled.complete.resolve();
    }
    await bounded(r.close()).catch(() => { });
    if (!r.journal.closed)
        r.journal.close();
}
for (const action of ['abort', 'close', 'failure'])
    test(`SQLite contention cannot prevent ${action} from stopping a generation`, timeout, async () => {
        ...
        try {
            ...
            if (action === 'failure')
                assert.match(result.errorMessage, /POLICY:.*selected model/);
            else
...
for (const timing of ['reply-received', 'rpc-pending'])
    test(`prompt deadline covers stalled observers with ${timing}`, timeout, async () => {
        const c = config();
        c.cli.promptTimeoutMs = 300;
        c.limits.maxHandoffMs = 200;
        c.admission.maxActive = 1;
        const r = new ProviderRuntime(c), entered = deferred(), gate = deferred();
        let callbacks = 0;
        try {
            ...
            gate.reject(new Error('Late hook rejection'));
            await delay(20);
...
test('abort closes every active binding even when lease bookkeeping fails', timeout, async () => {
    ...
    try {
        ...
        const bindings = [...r.bindings.values()];
        ...
        for (const answer of await bounded(Promise.all(answers))) {
            ...
            assert.match(answer.errorMessage, /cleanup failed.*locked/i);
        }
        other.db.exec('ROLLBACK');
        locked = false;
        await r.close();
...
test('one prompt deadline spans multiple Pi handoffs and accepts a late authoritative result', timeout, async () => {
    const c = config();
    c.cli.promptTimeoutMs = 2000;
    c.limits.maxHandoffMs = 1900;
...
test('prompt deadline also cancels a stalled pre-send hook', timeout, async () => {
    const c = config();
    c.cli.promptTimeoutMs = 100;
    const s = new V3Session(c, c.stateDir, new Catalog([])), gate = deferred();
    s.onEvent = async (event) => { if (event.kind === 'prompt_start')
        await gate.promise; };
    try {
        await s.start('Offline pre-send deadline', undefined);
        await s.select('test-opus', undefined);
        const pid = s.rpc.pid;
        await assert.rejects(bounded(s.prompt('No prompt should be sent')), { code: 'TIMEOUT' });
        await bounded(s.close());
        assert.equal(alive(pid), false);
        gate.reject(new Error('Late pre-send rejection'));
        await delay(10);
    }
    finally {
        gate.resolve();
...
test('failed runtime initialization closes the journal without masking the startup error', () => {
    ...
    Journal.prototype.reconcileDeadOwners = function () {
        opened = this;
        this.db.exec('PRAGMA busy_timeout=20');
        other.db.exec('BEGIN IMMEDIATE');
        locked = true;
        throw failure;
    };
...
test('inspection spawn failures remove the abort listener and settle cleanup', timeout, async () => {
    const c = config(), ctl = new AbortController();
    c.cli.binary = c.stateDir + '/nonexistent-kiro';
    try {
...
test('owned abandonment re-reads a concurrently recorded result before retiring it', () => {
    ...
    const unresolved = j.unresolved.bind(j);
    j.unresolved = binding => {
        const rows = unresolved(binding);
        other.transition(row.id, 'RESULT_RECORDED', 'real-result');
        return rows;
    };
    try {
        j.abandonOwned();
        assert.equal(j.get(row.id)?.phase, 'RESULT_RECORDED');
        assert.equal(j.get(row.id)?.result_hash, 'real-result');
    }
    finally {
        j.close();
        other.close();
...
function processes(dir) {
    const file = dir + '/inspect-pids.jsonl';
    return fs.existsSync(file) ? fs.readFileSync(file, 'utf8').trim().split('\n').filter(Boolean).map(line => JSON.parse(line)) : [];
}
async function stopped(rows) {
    await delay(40);
    const before = rows.map(row => fs.readFileSync(row.marker, 'utf8'));
    await delay(100);
    assert.deepEqual(rows.map(row => fs.readFileSync(row.marker, 'utf8')), before, 'inspection descendant continued running');
}
function killInspection(dir) {
    for (const row of processes(dir))
        for (const pid of [row.child, row.parent]) {
            try {
...
for (const action of ['abort', 'timeout', 'parent-exit', 'oversized'])
    test(`inspection cleans its process tree after ${action}`, { ...timeout, skip: process.platform === 'win32' }, async () => {
        const c = config(), ctl = new AbortController();
        c.cli.rpcTimeoutMs = 600;
        c.cli.cancelGraceMs = 100;
        ...
        const result = action === 'parent-exit' ? pending : assert.rejects(pending, { code: action === 'abort' ? 'CANCELLED' : action === 'timeout' ? 'TIMEOUT' : 'LIMIT' });
        try {
...
for (const phase of ['RESULT_RECORDED', 'RETURNED_TO_KIRO', 'CANCELLED'])
    test(`recovery preserves concurrently reconciled phase ${phase}`, () => {
        ...
        first.unresolved = binding => {
            ...
            if (!raced) {
                raced = true;
                second.transition(row.id, 'RESULT_RECORDED', 'real-result');
                if (phase !== 'RESULT_RECORDED')
                    second.transition(row.id, phase);
            }
            return rows;
        };
        try {
...
//# sourceMappingURL=reliability-faults.test.js.map