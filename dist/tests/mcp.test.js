...
function auth(server) {
    ...
    return Object.fromEntries(d.headers.map(h => [h.name, h.value]));
}
async function post(s, id, method, params, headers = {}) {
...
test('MCP rejects DNS rebinding Host header', async () => {
    ...
    await s.start();
    try {
        const status = await new Promise(resolve => { const req = http.request(s.url, { method: 'POST', headers: { ...auth(s), host: 'evil.invalid', 'content-type': 'application/json' } }, res => { res.resume(); resolve(res.statusCode); }); req.end('{}'); });
...
test('MCP responds with the exact active catalog and holds execution until Pi supplies a result', async () => {
    ...
    await s.start();
    try {
        const initialized = await (await post(s, 0, 'initialize', { protocolVersion: '2025-03-26' })).json();
        ...
        const catalog = await (await post(s, 1, 'tools/list', {})).json();
