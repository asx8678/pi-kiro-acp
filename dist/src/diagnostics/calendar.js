...
export function localDay(at, timeZone) {
...
export function validDay(day) {
    if (!/^[1-9]\d{3}-\d{2}-\d{2}$/.test(day))
        return false;
    const date = new Date(`${day}T00:00:00Z`);
    return Number.isFinite(date.getTime()) && date.toISOString().slice(0, 10) === day;
}
export function validMonth(month) { return /^\d{4}-\d{2}$/.test(month) && validDay(`${month}-01`); }
export function shiftMonth(month, count) {
    if (!validMonth(month))
        throw new BridgeError('CONFIG', 'Use a valid month: YYYY-MM.');
    const date = new Date(`${month}-01T00:00:00Z`);
    date.setUTCMonth(date.getUTCMonth() + count);
    const result = date.toISOString().slice(0, 7);
    if (!validMonth(result))
        throw new BridgeError('CONFIG', 'Month is outside the supported calendar.');
    return result;
}
