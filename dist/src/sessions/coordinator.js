...
import { deferred, hash, uid, withAbort, object, list } from '../util.js';
...
export class Binding {
    ...
    acceptResult(snap) {
        ...
        const result = { content, isError: resultMessage.isError === true };
    ...
    async run(snap, stream, options, taskId) {
        if (this.busy)
    ...
    async receiveTool(alias, args, ctx) {
        if (this.closing || this.fatal)
        ...
        const pending = { row: received.row, requestId: ctx.id, response: deferred() };
