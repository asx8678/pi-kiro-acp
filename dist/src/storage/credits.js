import { tokenFields } from '../kiro/tokens.js';
...
export class CreditLedger {
    ...
    cached(key, load) {
        ...
        if (afterVersion === version) {
            ...
            this.cache.clear();
            this.cacheVersion = afterVersion;
        }
    ...
    constructor(journal, scope, budget, timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', retainTaskExcerpts = false) {
        this.journal = journal;
        this.scope = scope;
        this.budget = budget;
        this.timeZone = timeZone;
        this.retainTaskExcerpts = retainTaskExcerpts;
    ...
    taskTokens(taskId, day) {
        return this.cached(`taskTokens:${JSON.stringify([taskId, day])}`, () => this.tokenTotals('SELECT scope,id FROM credit_prompts WHERE scope=? AND task_id=?' + (day ? ' AND started_at>=? AND started_at<?' : ''), [this.scope, taskId, ...(day ? dayRange(day, this.timeZone) : [])]));
    }
    promptTokens(promptId, day) {
        return this.cached(`promptTokens:${JSON.stringify([promptId, day])}`, () => this.tokenTotals('SELECT scope,id FROM credit_prompts WHERE scope=? AND id=?' + (day ? ' AND started_at>=? AND started_at<?' : ''), [this.scope, promptId, ...(day ? dayRange(day, this.timeZone) : [])]));
    }
    sessionTokens(sessionId, day) {
        ...
        return this.cached(`sessionTokens:${JSON.stringify([sessionId, day])}`, () => this.tokenTotals(`
            ...
            AND NOT EXISTS (SELECT 1 FROM credit_tasks t WHERE t.scope=p.scope AND t.id=p.task_id)`, [this.scope, sessionId, ...range, this.scope, sessionId, ...range]));
    }
    tokenUsage(sessionId) {
        return this.cached(`tokenUsage:${sessionId}`, () => this.readTokenUsage(sessionId));
    }
    readTokenUsage(sessionId) {
    ...
    readSnapshot(now) {
        ...
        const row = this.journal.db.prepare(`SELECT SUM(credits) AS used, COUNT(*) AS prompts,
            ...
            .get(this.scope, start, end);
        const all = this.journal.db.prepare('SELECT SUM(credits) AS used FROM credit_prompts WHERE scope=?').get(this.scope);
    ...
    totals(start = 0, end = 8640000000000000) {
        return this.journal.db.prepare(`SELECT SUM(credits) AS credits,COUNT(*) AS prompts,
            ...
            FROM credit_prompts WHERE scope=? AND started_at>=? AND started_at<?`).get(this.scope, start, end);
    }
    dashboard(selection, now = Date.now()) {
        return this.cached(`dashboard:${selection ?? ''}:${localDay(now, this.timeZone)}`, () => this.readDashboard(selection, now));
    }
    readDashboard(selection, now) {
        ...
        if (!validMonth(selected) && !validDay(selected))
            throw new BridgeError('CONFIG', 'Usage: /usage [YYYY-MM | YYYY-MM-DD | refresh]');
        ...
        const days = this.journal.db.prepare(`WITH days(day,start,end) AS (VALUES ${bounds.map(() => '(?,?,?)').join(',')})
            ...
            .all(...bounds.flatMap(({ day, start, end }) => [day, start, end]), this.scope);
        let sessions = [];
        if (validDay(selected)) {
            sessions = this.journal.db.prepare(`SELECT COALESCE(t.session_id,p.owner_instance) AS id,
                ...
                .all(this.scope, ...dayRange(selected, this.timeZone));
        }
    ...
    readSessionTasks(day, sessionId) {
        return this.journal.db.prepare(`SELECT COALESCE(t.id,p.id) AS id,CASE WHEN t.id IS NULL THEN 'prompt' ELSE 'task' END AS kind,COALESCE(t.summary,'Provider request') AS summary,
            ...
            .all(this.scope, ...dayRange(day, this.timeZone), sessionId);
    }
    assertAvailable() {
        ...
        const row = this.journal.db.prepare('SELECT SUM(credits) AS used FROM credit_prompts WHERE scope=? AND started_at>=? AND started_at<?')
            .get(this.scope, ...dayRange(day, this.timeZone));
