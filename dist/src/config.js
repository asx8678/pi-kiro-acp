...
import { object, boundedString } from './util.js';
export function agentDir() { return path.resolve(process.env.PI_CODING_AGENT_DIR || path.join(os.homedir(), '.pi', 'agent')); }
export function configPath() { return path.resolve(process.env.PI_KIRO_ACP_CONFIG || path.join(agentDir(), 'kiro-acp.json')); }
...
function keys(input, allowed, label) {
...
export function parseConfig(raw) {
    ...
    for (const [label, values] of [['cli.prefixArgs', c.cli.prefixArgs], ['compatibility.approvedVersions', c.compatibility.approvedVersions], ['compatibility.orderedSteeringVersions', c.compatibility.orderedSteeringVersions]]) {
    ...
    for (const section of ['cli', 'models', 'sessions', 'limits', 'admission', 'efficiency', 'reporting']) {
        const value = c[section];
        const template = d[section];
    ...
    for (const section of ['compatibility', 'sessions', 'policy', 'efficiency', 'reporting']) {
        const v = c[section], base = d[section];
    ...
    try {
        new Intl.DateTimeFormat('en', { timeZone: c.reporting.timeZone }).format();
    }
    catch {
        throw new BridgeError('CONFIG', 'reporting.timeZone must be a valid IANA time zone.');
    }
